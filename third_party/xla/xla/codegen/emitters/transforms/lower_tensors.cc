  ...
  Value sub_byte_index = b.create<mlir::arith::TruncIOp>(
      ...
      b.create<mlir::arith::AndIOp>(
          linear_index, b.create<mlir::arith::ConstantIntOp>(
                            linear_index.getType(), ((1 << bit_width) - 1))));
  // This is the shift required to get the sub element into the bit_width lsbs.
  // A slightly easier to understand formulation is:
  // 8 - width * (index + 1), but this requires an extra runtime addition.
  Value sub_byte_shift = b.create<mlir::arith::SubIOp>(
      b.create<mlir::arith::ConstantIntOp>(b.getI8Type(), 8 - bit_width),
    ...
    if (sub_byte_width) {
      ...
      auto tensor_dest_i8 =
          b.create<UnrealizedConversionCastOp>(tensor_ty, tensor_dest)
              .getResult(0);
      if (scalar_value.getType() != element_type) {
        scalar_value = b.create<arith::BitcastOp>(element_type, scalar_value);
      }
